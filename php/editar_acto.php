...
// Igual sanitización y validación que guardar_acto.php
...
if ($errores) die("Errores: " . implode(", ", $errores));

try {
    $pdo->beginTransaction();

    // Actualizar acto
    $stmt = $pdo->prepare("UPDATE acto_sacramental SET persona_id=?, sacramento_id=?, parroquia_id=?, parroco_id=?, fecha=?, libro=?, folio=?, partida=?, observaciones=?, certificado_emitido=? WHERE id=?");
    $stmt->execute([
        $persona_id, $sacramento_id, $parroquia_id ?: null, $parroco_id ?: null, $fecha, $libro ?: null, $folio ?: null, $partida ?: null, $observaciones ?: null, $certificado_emitido, $id
    ]);

    // Eliminar relaciones viejas
    $pdo->prepare("DELETE FROM relaciones_persona WHERE acto_sacramental_id=?")->execute([$id]);

    // Guardar relaciones nuevas si vienen
    if (!empty($_POST['relaciones']) && is_array($_POST['relaciones'])) {
        ...
        foreach ($_POST['relaciones'] as $rel) {
            ...
            if ($pid && in_array($rol, ['padre', 'madre', 'padrino', 'madrina'])) {
                $stmtRel->execute([$id, $pid, $rol]);
            }
        }
    }

    $pdo->commit();
    header("Location: ../admin/acto.php?msg=editado");
    exit;
} catch (Exception $e) {
    $pdo->rollBack();
